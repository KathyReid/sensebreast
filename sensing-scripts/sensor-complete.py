...
# fromtimestamp(time.time()) is equivalent.

# loop for the scriptDuration
while datetime.now() < (startTime + scriptDuration):
    ...
    data["meta"].append({"timestamp": timestamp, "datestamp": datestamp})

    # make this a dict literal with commas

    # temperature and pressure and humidity readings
    temperature = sense.get_temperature()
    # print("Temperature: {t:.5f}".format(t=temperature, precision=5))
    data["basic"].append({"temperature": temperature})

    pressure = sense.get_pressure()
    data["basic"].append({"pressure": pressure})
    # print("Pressure: {p: .5f}".format(p=pressure, precision=5))

    humidity = sense.get_humidity()
    data["basic"].append({"humidity": humidity})
    ...
    data["imu"]["orientation"].append({"pitch": pitch, "roll": roll, "yaw": yaw})
...
fileName = (
    "/home/pi/sensebreast/sensing-scripts/sensor-readings/" + str(time.time()) + ".json"
)
